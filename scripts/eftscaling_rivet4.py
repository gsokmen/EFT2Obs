...
class EFTScaling(object):
    ...
    def makeTH1Prototype(self, name):
        ...
        label_list = []
        if self.is3D():
            # For 3D histograms: use flat bin index as x-axis
            edges = array('d', [float(i) for i in range(len(self.bin_edges) + 1)])
            label_list = ['%g-%g,%g-%g,%g-%g' % tuple(X) for X in self.bin_edges]
    ...
    def is3D(self):
        # 3D bins stored as flat [xlo,xhi,ylo,yhi,zlo,zhi] (6 elements, all floats)
        return (not self.is2D() and len(self.bin_edges) > 0
                and len(self.bin_edges[0]) == 6)
    ...
    def writeToTxt(self, filename, translate_txt=dict()):
        ...
        for ib in range(self.nbins):
            if self.is3D():
                bin_label = '%g-%g,%g-%g,%g-%g' % tuple(self.bin_edges[ib])
            elif self.is2D():
    ...
    def writeToTex(self, filename, translate_tex=dict()):
        ...
        for ib in range(self.nbins):
            if self.is3D():
                line = '$%g$--$%g$, $%g$--$%g$, $%g$--$%g$ & ' % tuple(self.bin_edges[ib])
            elif self.is2D():
